...
from models import app, db, SECRET_KEY, Employer, Employees
# app= Flask(__name__) 
# db= SQLAlchemy(app)
...
def upload_get_all():
    if request.method == 'POST':
        ...
        email = body['email']
        data=Employer(company, first_name, last_name, email)
        db.session.add(data)
        db.session.commit()
        return jsonify({
...
def info_per_id(id):
    ...
    if request.method=='PUT':
        ...
        allinfo=Employer.query.filter_by(id=id).first()
